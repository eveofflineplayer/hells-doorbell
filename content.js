...
style.textContent = `
  ...
  .historyButton{
    color: #a53103;
    border-left: 1px solid #a53103;
    border-right: 1px solid #a53103;
    transition: all 0.25s ease;
  }
  .historyButton:hover {
    color: #ce3a00;
    border-left: 1px solid #ce3a00;
    border-right: 1px solid #ce3a00; 
  }

  .pulsating-glow {
    animation: smooth-glow 1.5s infinite cubic-bezier(0.4, 0, 0.6, 1);
    box-sizing: border-box;
    border-radius: 3px;
  }
  .pulsating-glow-friendly {
    animation: smooth-glow-friendly 1.5s infinite cubic-bezier(0.4, 0, 0.6, 1);
    box-sizing: border-box;
    border-radius: 3px;
  }
  ...
  @keyframes smooth-glow {
    0%, 100% {
      box-shadow: 0 0 3px #a53103,
                  inset 0 0 2px #a53103;
      border: 1px solid #a53103;
    }
    50% {
      box-shadow: 0 0 12px 5px rgba(165, 49, 3, 0.7),
                  inset 0 0 7px rgba(165, 49, 3, 0.5);
      border: 2px solid #a53103;
    }
  }
  @keyframes smooth-glow-friendly {
    0%, 100% {
    ...
    50% {
      box-shadow: 0 0 12px 5px #5cb85cc0,
                  inset 0 0 7px #5cb85cc0;
...
let knownHoles = {
  "J111740": "CUBA",
  "J214811": "Zoo Landers",
  "J130451": "Band of Magnus",
  "J142214": "Garden of Shadows",
  "J230959": "Stryker Ind.",
  "J110417": "Shadow Panthers",
  "J160941": "Florida DKVC",
  "J005280": "Minus's dad's spot",
  "J100447": "Varadero",
  "J231837": "Lux Collective",
}
fetch("https://gist.githubusercontent.com/eveofflineplayer/5df8c1a3d5ac589789f726d1510f8726/raw/jcodes.json").then(res=>res.json()).then(json=>{knownHoles = json})

async function fetchFinalBlowName(esiLink) {
  try{
    const res = await fetch(esiLink);
    const json = await res.json();
    const finalBlow = json.attackers.find(it=>it.final_blow)
    const charRes = await fetch(`https://esi.evetech.net/characters/${finalBlow.character_id}`);
...
function highlightSystem(system, isFriendly){
  const element = document.querySelector(`[data-value="${system}"]`)
  const systemParent = element?.parentElement?.parentElement
  if(!systemParent){
    return;
  }
  const classToAdd = isFriendly ? 'pulsating-glow-friendly' : 'pulsating-glow'
  systemParent.classList.add(classToAdd);
  setTimeout(() => {
    systemParent.classList.remove(classToAdd)
  }, 60000);
  systemParent.addEventListener("click", e => {
    systemParent.classList.remove(classToAdd)
  })
}

function stopHighlightSystem(system){
  const element = document.querySelector(`[data-value="${system}"]`)
  const systemParent = element?.parentElement?.parentElement
  if(!systemParent){
    return;
  }
  systemParent.classList.remove("pulsating-glow-friendly")
...
const app = async () => {
  ...
  setTimeout(() => {
    ...
    if (chain) {
      console.log("Chain exists, observing");
      
      const observer = new MutationObserver(() => {
        console.log("Mutation detected, scraping chain");
        scrapeInfo()
      });
      observer.observe(chain, { childList: true, subtree: true });
    }
  }, 5000);
  setTimeout(() => {
    scrapeInfo();
  ...
  const scrapeInfo = () => {
    ...
    const newSystems = {};
    nodes.forEach((node) => {
      const systemSec = node.querySelector('.pf-system-sec');
      const systemTag = node.querySelector('.pf-system-head-tag');
      
      if(!systemSec || !systemTag){
        return;
      }
      const name = systemSec.innerText + systemTag.innerText;

      const systemName = node.querySelector('.pf-system-head-name');
      if(!systemName){
        return
      }
      const jcode = systemName.innerText;
      
        newSystems[jcode] = name;
    });
    

    if (!haveSameKeys(savedSystems, newSystems)) {
  ...
  function createHistory() {
    ...
    historyWindow.style.cursor = "default";
    historyWindow.style.width = "max-content";
    historyWindow.style.maxHeight = "400px";
    historyWindow.style.height = "max-content";
    historyWindow.style.display = "none";
    historyWindow.style.flexDirection = "column";
    historyWindow.style.justifyContent = "flex-start";
    historyWindow.style.alignItems = "center";
    historyWindow.style.backgroundColor = "#1f2324";
    historyWindow.style.position = "absolute";
    historyWindow.style.top = "50px";
    historyWindow.style.left = "0px";
    ...
    volumeButton.style.height = "25px";
    volumeButton.style.borderRadius = "5px";
    volumeButton.style.backgroundColor = "white";
    volumeButton.style.border = "1px solid black";
    ...
    volumeButton.style.userSelect = "none";
    volumeButton.style.boxShadow = "3px 3px 6px #000";
    const volume = localStorage.getItem("volume");
    if (volume) {
      volumeButton.innerText = `Volume: ${volume.replace("0.", "")}%`;
    }
    volumeButton.addEventListener("click", (e) => {
      const newVolume = Math.floor(Math.random() * 100) / 100;
      localStorage.setItem("volume", `${newVolume}`);
      volumeButton.innerText = `Volume: ${newVolume
        .toString()
        .replace("0.", "")}%`;
      playSound("ding.wav");
    });
    ...
    clear.style.boxShadow = "3px 3px 6px #000";
    clear.addEventListener("click", (e) => {
      localStorage.setItem("killHistory", JSON.stringify([]));
      window.location.reload();
    });

    const togglePosition = document.createElement("div");
    const position = localStorage.getItem("position") ?? "Left";
    togglePosition.classList.add("killButton");
    togglePosition.classList.add("zkill");
    togglePosition.innerText = position === "Left" ? "Right" : "Left";
    togglePosition.addEventListener("click", (e) => {

      const newPosition = localStorage.getItem("position") ?? "Left";
      if(newPosition === "Left"){
        historyButton.classList.remove("pull-left")
        historyButton.classList.add("pull-right")
        historyWindow.style.left = "unset";
        historyWindow.style.right = "0px"
        localStorage.setItem("position", "Right")
        togglePosition.innerText = "Left";

      }else{
        historyButton.classList.remove("pull-right")
        historyButton.classList.add("pull-left")
        historyWindow.style.left = "0px";
        historyWindow.style.right = "unset";
        localStorage.setItem("position", "Left")
        togglePosition.innerText = "Right";
      }
    });

    const bottomTab = document.createElement("div");
    bottomTab.style.width = "100%";
    bottomTab.style.height = "50px";
    bottomTab.style.display = "flex";
    bottomTab.style.justifyContent = "space-evenly";
    bottomTab.style.alignItems = "center";

    bottomTab.appendChild(clear);
    bottomTab.appendChild(volumeButton);
    bottomTab.appendChild(togglePosition);

    historyWindow.appendChild(bottomTab);

    const historyButton = document.createElement("div");
    historyButton.classList.add("historyButton");
    historyButton.style.width = "100px";
    historyButton.style.height = "30px";
    historyButton.innerText = "History";
    historyButton.style.fontWeight = "bold";
    historyButton.style.zIndex = "101";
    ...
    historyButton.addEventListener("click", (e) => {
      ...
      if (historyWindow.style.display === "flex") {
        historyWindow.style.display = "none";
      } else {
        historyWindow.style.display = "flex";
      }
    });
    historyButton.appendChild(historyWindow);

    setTimeout(() => {
      const topContainer = document.querySelector(".container-fluid");
      topContainer.insertBefore(historyButton, topContainer.children[1])
    }, 1000);
  ...
  function createHistoryEntry(historyWindow, kill) {
    ...
    entry.addEventListener("mouseleave", e=>{
      stopHighlightSystem(kill.systemId)
    })
    ...
    const zkillButton = document.createElement("div");
    zkillButton.classList.add("killButton");
    zkillButton.classList.add("zkill");
    zkillButton.innerText = "Zkill";
    zkillButton.addEventListener("click", (e) => {
      window.open(kill.kill, "_blank");
    });

    const apexButton = document.createElement("div");
    apexButton.classList.add("killButton");
    apexButton.classList.add("apex");
    apexButton.innerText = "Apex";
    apexButton.addEventListener("click", (e) => {
      window.open(kill.apexLink, "_blank");
    });
    
    buttons.append(zkillButton)
    buttons.append(apexButton)
    entry.append(buttons)
    historyWindow.prepend(entry);
  }
  ...
  async function onMessage(kill) {
    try {
      ...
      if(knownHoles[kill.system]){
        location = knownHoles[kill.system]
      }
      if(location === "HS"){
        return;
      }
      ...
      if(systems[location]){
        ...
        playSound("ding.wav");
        createSnack(snackInfo, url, isFriendly);
        highlightSystem(location, isFriendly);
        const history = JSON.parse(localStorage.getItem("killHistory"));
        if (history) {
          history.push({
            apexLink,
          ...
          localStorage.setItem("killHistory", JSON.stringify(history));
        } else {
        ...
        createHistoryEntry(historyWindow, {
          apexLink,
          isFriendly,
          snackInfo,
          name: systems[location],
